...
/*
 * COPIED FROM AOSP: KSize (starts in API 21)
 *
...
public final class KSize {
     ...
     * <p>
     * For any {@code KSize s}: {@code KSize.parseSize(s.toString()).equals(s)}.
     * However, the method also handles sizes expressed in the
     * following forms:</p>
     * <p>
     * "<i>width</i>{@code x}<i>height</i>" or
     * "<i>width</i>{@code *}<i>height</i>" {@code => new KSize(width, height)},
     * where <i>width</i> and <i>height</i> are string integers potentially
     * containing a sign, such as "-10", "+7" or "5".</p>
     *
     * <pre>{@code
     * KSize.parseSize("3*+6").equals(new KSize(3, 6)) == true
     * KSize.parseSize("-3x-6").equals(new KSize(-3, -6)) == true
     * KSize.parseSize("4 by 3") => throws NumberFormatException
     * }</pre>
     *
    ...
    public static KSize parseSize(String string)
        ...
        try {
            return new KSize(Integer.parseInt(string.substring(0, sep_ix)),
